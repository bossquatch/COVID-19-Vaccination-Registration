...
class AnalyticsController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('analytics.index', [
            ...
            'registrations_today' 	=> DB::select('SELECT COUNT(*) `Count` FROM registrations WHERE `deleted_at` IS NULL AND DATE(`submitted_at`) = DATE(DATE_ADD(NOW(), INTERVAL -5 HOUR))')[0]->Count,
            'registrations_total' 	=> DB::select('SELECT COUNT(*) `Count` FROM registrations WHERE `deleted_at` IS NULL')[0]->Count,
            'registrations_old' 	=> DB::select('SELECT COUNT(*) `Count` FROM registrations WHERE `deleted_at` IS NULL AND DATE(`birth_date`) <= DATE_SUB(CURDATE(), INTERVAL 65 YEAR)')[0]->Count,
            'registrations_young' 	=> DB::select('SELECT COUNT(*) `Count` FROM registrations WHERE `deleted_at` IS NULL AND DATE(`birth_date`) > DATE_SUB(CURDATE(), INTERVAL 65 YEAR)')[0]->Count,
        ]);
    }
